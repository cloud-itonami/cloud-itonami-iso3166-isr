# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Israel

## Classification

- Repository: `cloud-itonami-iso3166-isr`
- ISO 3166: `ISR` (Israel)
...
- Social impact: [:israeli-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on an Israeli
  public contract
...
- registration walkthrough for the Government Procurement Administration
  (GPA, mr.gov.il), which operates Israel's digital tender portal and
  manages procurement processes from initiation through tender-committee
  and bid-submission workflows
- business/tax registration checklist: an entry with the Registrar of
  Companies (Rasham HaChavarot), administered by the Israel Corporations
  Authority under the Ministry of Justice
- local-content / preferential-procurement navigation: Israeli
  public-procurement preference provisions favoring local manufacturers
  and Israeli-owned suppliers on qualifying tenders under the Mandatory
  Tenders Law
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Israeli-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Israel).
