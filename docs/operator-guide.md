...
2. Register the client's intake: business type, target public function,
   prior filing history in Israel if any.
3. Run the advisor in read-only mode against the Government Procurement
   Administration's (GPA) digital tender portal.
4. Compare the checklist against the client's current documentation
   (Registrar of Companies status).
...
- named referral relationship with Israeli-licensed counsel or a
...
a working referral relationship with Israeli-licensed counsel or a
registered agent for whatever licensed representation the law of
Israel requires for actual public-procurement filings.
