(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Israel-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Israel-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-il-entity?
                                       true` (near-universal for Israel
                                       ...
                                       Grounded in Israeli company registration
                                       typically required for government awards
                                       (contracting authorities / Accountant
                                       General).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(COMPANY-NO/統一資格/Merkava/e-procurement登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- il-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-il-entity? true`, INDEPENDENTLY verify
  `:has-il-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-il-entity?` ground truth (most Israel public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-il-entity? e))
                 (not (true? (:has-il-entity? e))))
        [{:rule :il-entity-missing
          :detail (str subject " はイスラエル居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- company-no-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-company-no? true`, INDEPENDENTLY check
  `:company-no-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-company-no? e))
                 (not (true? (:company-no-verified? e))))
        [{:rule :company-no-unverified
          :detail (str subject " はCOMPANY-NO確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (il-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (company-no-unverified-violations request st)
