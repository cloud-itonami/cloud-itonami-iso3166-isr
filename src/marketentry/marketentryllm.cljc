(ns marketentry.marketentryllm
  ...
  the ISR public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-il-entity?=" (:has-il-entity? e)
                        " company-no-verified?=" (:company-no-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-il-entity? e))
                              (:has-il-entity? e))
                          (or (not (:requires-company-no? e))
                              (:company-no-verified? e)))
                   0.9 0.3)}))
